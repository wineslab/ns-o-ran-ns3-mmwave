...
EnbRrcMemberLteEnbCmacSapUser::EnbRrcMemberLteEnbCmacSapUser (LteEnbRrc *rrc,
    ...
    : m_rrc (rrc), m_componentCarrierId{componentCarrierId}
{
}
  ...
  for (uint16_t i = 0; i < m_rrc->m_numberOfComponentCarriers; i++)
    {
      ...
      m_rrc->m_cphySapProvider.at (i)->SetTransmissionMode (
          m_rnti, m_physicalConfigDedicated.antennaInfo.transmissionMode);
      m_rrc->m_cphySapProvider.at (i)->SetSrsConfigurationIndex (
          m_rnti, m_physicalConfigDedicated.soundingRsUlConfigDedicated.srsConfigIndex);
    }
  ...
  switch (m_state)
    ...
    case INITIAL_RANDOM_ACCESS:
      m_connectionRequestTimeout =
          Simulator::Schedule (m_rrc->m_connectionRequestTimeoutDuration,
                               &LteEnbRrc::ConnectionRequestTimeout, m_rrc, m_rnti);
      break;

    case HANDOVER_JOINING:
      m_handoverJoiningTimeout =
          Simulator::Schedule (m_rrc->m_handoverJoiningTimeoutDuration,
                               &LteEnbRrc::HandoverJoiningTimeout, m_rrc, m_rnti);
      break;
  ...
  static TypeId tid =
      TypeId ("ns3::UeManager")
          ...
          .AddAttribute ("DataRadioBearerMap", "List of UE DataRadioBearerInfo by DRBID.",
                         ObjectMapValue (), MakeObjectMapAccessor (&UeManager::m_drbMap),
                         MakeObjectMapChecker<LteDataRadioBearerInfo> ())
          .AddAttribute ("DataRadioRlcMap", "List of UE Secondary RLC by DRBID.", ObjectMapValue (),
                         MakeObjectMapAccessor (&UeManager::m_rlcMap),
                         MakeObjectMapChecker<RlcBearerInfo> ())
          .AddAttribute ("Srb0", "SignalingRadioBearerInfo for SRB0", PointerValue (),
                         MakePointerAccessor (&UeManager::m_srb0),
                         MakePointerChecker<LteSignalingRadioBearerInfo> ())
          .AddAttribute ("Srb1", "SignalingRadioBearerInfo for SRB1", PointerValue (),
          ...
          .AddTraceSource ("SecondaryRlcCreated", "fired upon receiving RecvRlcSetupRequest",
...
void
UeManager::SetupDataRadioBearer (EpsBearer bearer, uint8_t bearerId, uint32_t gtpTeid,
                                 Ipv4Address transportLayerAddress)
{
  ...
  NS_ASSERT_MSG (bearerId == 0 || bid == bearerId,
                 "bearer ID mismatch (" << (uint32_t) bid << " != " << (uint32_t) bearerId
                                        << ", the assumption that ID are allocated in the same way "
                                           "by MME and RRC is not valid any more");
...
UeManager::StartDataRadioBearers ()
{
  ...
  for (std::list<uint8_t>::iterator drbIdIt = m_drbsToBeStarted.begin ();
       drbIdIt != m_drbsToBeStarted.end (); ++drbIdIt)
    {
...
UeManager::MergeBuffers (std::vector<LteRlcAm::RetxPdu> first,
...
UeManager::RecvHandoverRequestAck (EpcX2SapUser::HandoverRequestAckParams params)
{
  ...
  m_handoverLeavingTimeout = Simulator::Schedule (
      m_rrc->m_handoverLeavingTimeoutDuration, &LteEnbRrc::HandoverLeavingTimeout, m_rrc, m_rnti);
  NS_ASSERT (handoverCommand.haveMobilityControlInfo);
  m_rrc->m_handoverStartTrace (m_imsi, m_rrc->ComponentCarrierToCellId (m_componentCarrierId),
                               m_rnti, handoverCommand.mobilityControlInfo.targetPhysCellId);
  ...
  for (std::map<uint8_t, Ptr<LteDataRadioBearerInfo>>::iterator drbIt = m_drbMap.begin ();
       drbIt != m_drbMap.end (); ++drbIt)
    {
      ForwardRlcBuffers (drbIt->second->m_rlc, drbIt->second->m_pdcp, drbIt->second->m_gtpTeid, 0,
                         0, 0);
    }
...
UeManager::ForwardRlcBuffers (Ptr<LteRlc> rlc, Ptr<LtePdcp> pdcp, uint32_t gtpTeid,
                              bool mcLteToMmWaveForwarding, bool mcMmToMmWaveForwarding,
                              uint8_t bid)
{
      ...
      if (rlcAm->GetTransmittingRlcSduBufferSize () > 0)
        { //something inside the RLC AM's transmitting buffer
          ...
          for (std::map<uint32_t, Ptr<Packet>>::iterator it = rlcAmTransmittingBuffer.begin ();
               it != rlcAmTransmittingBuffer.end (); ++it)
            {
              if (it->second != 0)
                {
          ...
          m_x2forwardingBuffer.insert (m_x2forwardingBuffer.end (), txonBuffer.begin (),
                                       txonBuffer.end ());
...
UeManager::SendUeContextRelease ()
{
  ...
  switch (m_state)
    ...
    case HANDOVER_PATH_SWITCH:
      ...
      m_rrc->m_handoverEndOkTrace (m_imsi, m_rrc->ComponentCarrierToCellId (m_componentCarrierId),
                                   m_rnti);
      break;
  ...
  if (m_isMc)
    {
      ...
      std::vector<LteCcmRrcSapProvider::LcsConfig> lcOnCcMapping =
          m_rrc->m_ccmRrcSapProvider->SetupDataRadioBearer (bearer, bid, m_rnti, lcid,
                                                            m_rrc->GetLogicalChannelGroup (bearer),
                                                            rlc->GetLteMacSapUser ());
      // LteEnbCmacSapProvider::LcInfo lcinfo;
      // lcinfo.rnti = m_rnti;
        ...
        if (m_rrc->m_admitRrcConnectionRequest == true)
          {
            ...
            m_connectionSetupTimeout =
                Simulator::Schedule (m_rrc->m_connectionSetupTimeoutDuration,
...
UeManager::RecvRrcConnectionSetupCompleted (LteRrcSap::RrcConnectionSetupCompleted msg)
{
  ...
  switch (m_state)
    ...
    case CONNECTION_SETUP:
      ...
      SwitchToState (CONNECTED_NORMALLY);
      m_rrc->m_connectionEstablishedTrace (
  ...
  switch (m_state)
              ...
              if (pdcp != 0)
                {
                  ...
                  ForwardRlcBuffers (it->second->m_rlc, pdcp, it->second->m_gtpTeid, 1, 0,
                                     it->first);
                }
              else
                {
    ...
    case CONNECTION_RECONFIGURATION:
      ...
      if (m_receivedLteMmWaveHandoverCompleted)
        {
          NS_LOG_INFO ("Notify LteEnbRrc that LTE cell received a NotifyLteMmWaveHandoverCompleted "
                       "and has completed CONNECTION_RECONFIGURATION");
...
UeManager::RecvMeasurementReport (LteRrcSap::MeasurementReport msg)
{
  ...
  if (msg.measResults.haveScellsMeas == true)
    {
      for (std::list<LteRrcSap::MeasResultScell>::iterator it =
           ...
           it != msg.measResults.measScellResultList.measResultScell.end (); ++it)
        {
  ...
  m_rrc->m_recvMeasurementReportTrace (
      m_imsi, m_rrc->ComponentCarrierToCellId (m_componentCarrierId), m_rnti, msg);
  ...
  for (std::map<uint8_t, Ptr<LteDataRadioBearerInfo>>::iterator it = m_drbMap.begin ();
       it != m_drbMap.end (); ++it)
    {
      if (!(it->second->m_isMc) ||
          ...
          if (pdcp != 0)
            {
              ...
              std::pair<std::map<uint32_t, LteEnbRrc::X2uTeidInfo>::iterator, bool> ret;
              ret = m_rrc->m_x2uMcTeidInfoMap.insert (
                  std::pair<uint32_t, LteEnbRrc::X2uTeidInfo> (it->second->m_gtpTeid, x2uTeidInfo));
              // NS_ASSERT_MSG (ret.second == true, "overwriting a pre-existing entry in m_x2uMcTeidInfoMap");
              // Setup McEpcX2PdcpUser
              m_rrc->m_x2SapProvider->SetEpcX2PdcpUser (it->second->m_gtpTeid,
                                                        pdcp->GetEpcX2PdcpUser ());
        ...
        {
          NS_LOG_INFO (
              "No difference with the MC Bearer already defined"); // TODO consider bearer modifications
        }
    }
              ...
              if (!m_rrc->m_ismmWave && useMmWaveConnection) //
                {
                  ...
                  ForwardRlcBuffers (it->second->m_rlc, it->second->m_pdcp, it->second->m_gtpTeid,
                                     1, 0, it->first);
                  ...
                  std::vector<LteCcmRrcSapProvider::LcsConfig> lcOnCcMapping =
                      m_rrc->m_ccmRrcSapProvider->SetupDataRadioBearer (
                          it->second->m_epsBearer, bid, m_rnti, lcid,
                  ...
                  for (itLcOnCcMapping = lcOnCcMapping.begin ();
                       itLcOnCcMapping != lcOnCcMapping.end (); ++itLcOnCcMapping)
                    {
                      NS_LOG_DEBUG (this << " RNTI " << itLcOnCcMapping->lc.rnti << "Lcid "
                                         << (uint16_t) itLcOnCcMapping->lc.lcId << " lcGroup "
  ...
  if (!useMmWaveConnection)
    {
      ...
      ForwardRlcBuffers (m_rlcMap.find (drbid)->second->m_rlc, 0,
                         m_rlcMap.find (drbid)->second->gtpTeid, 0, 0, 0);
      // create a new rlc!
      ...
      std::vector<LteCcmRrcSapProvider::LcsConfig> lcOnCcMapping =
          m_rrc->m_ccmRrcSapProvider->SetupDataRadioBearer (bearer, Drbid2Bid (drbid), m_rnti, lcid,
                                                            m_rrc->GetLogicalChannelGroup (bearer),
                                                            rlc->GetLteMacSapUser ());
...
UeManager::SwitchToState (State newState)
{
  ...
  m_stateTransitionTrace (m_imsi, m_rrc->ComponentCarrierToCellId (m_componentCarrierId), m_rnti,
                          oldState, newState);
                    ...
                    if (maxSinrDb > m_rrc->m_outageThreshold)
                      {
                        ...
                        m_rrc->m_rrcSapUser->SendRrcConnectToMmWave (
                            m_rnti, m_rrc->m_bestMmWaveCellForImsiMap.find (m_imsi)->second);
                      }
                    else
                      {
                        //TODO
          ...
          if (true)
            {
              ...
              component.radioResourceConfigDedicateSCell.physicalConfigDedicatedSCell.antennaInfoUl
                  .transmissionMode = m_rrc->m_defaultTransmissionMode;
              component.radioResourceConfigDedicateSCell.physicalConfigDedicatedSCell
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid =
      TypeId ("ns3::LteEnbRrc")
          ...
          .AddAttribute ("UeMap", "List of UeManager by C-RNTI.", ObjectMapValue (),
                         MakeObjectMapAccessor (&LteEnbRrc::m_ueMap),
                         MakeObjectMapChecker<UeManager> ())
          .AddAttribute ("DefaultTransmissionMode", "The default UEs' transmission mode (0: SISO)",
          ...
          .AddAttribute (
              "SrsPeriodicity", "The SRS periodicity in milliseconds", UintegerValue (320),
          ...
          .AddAttribute ("HandoverLeavingTimeoutDuration",
                         ...
                         MakeTimeChecker ())
          .AddAttribute ("OutageThreshold", "SNR threshold for outage events [dB]",
                         DoubleValue (-5.0), MakeDoubleAccessor (&LteEnbRrc::m_outageThreshold),
          ...
          .AddAttribute ("QRxLevMin",
                         ...
                         TypeId::ATTR_GET | TypeId::ATTR_CONSTRUCT, IntegerValue (-70),
                         MakeIntegerAccessor (&LteEnbRrc::m_qRxLevMin),
                         MakeIntegerChecker<int8_t> (-70, -22))
          .AddAttribute ("NumberOfComponentCarriers", "Number of Component Carriers ",
                         UintegerValue (1),
          ...
          .AddAttribute (
              ...
              BooleanValue (true), MakeBooleanAccessor (&LteEnbRrc::m_admitHandoverRequest),
              MakeBooleanChecker ())
          .AddAttribute ("AdmitRrcConnectionRequest",
                         "Whether to admit a connection request from a UE", BooleanValue (true),
          ...
          .AddAttribute ("RsrqFilterCoefficient",
                         ...
                         MakeUintegerChecker<uint8_t> (0))
          .AddAttribute ("mmWaveDevice", "Indicates whether RRC is for mmWave base station",
                         BooleanValue (false), MakeBooleanAccessor (&LteEnbRrc::m_ismmWave),
                         MakeBooleanChecker ())
          .AddAttribute ("FirstSibTime", "Time in ms of initial SIB message",
                         // i.e. the variable k in 3GPP TS 36.331 section 5.5.3.2
                         UintegerValue (2), MakeUintegerAccessor (&LteEnbRrc::m_firstSibTime),
                         MakeUintegerChecker<uint32_t> (0))
          .AddAttribute ("InterRatHoMode",
                         "Indicates whether RRC is for LTE base station that coordinates "
                         "InterRatHo among eNBs",
                         BooleanValue (false), MakeBooleanAccessor (&LteEnbRrc::m_interRatHoMode),
                         MakeBooleanChecker ())
          .AddAttribute (
              "HoSinrDifference", "The value for which an handover between MmWave eNB is triggered",
          ...
          .AddAttribute ("SecondaryCellHandoverMode", "Select the secondary cell handover mode",
                         ...
                         MakeEnumChecker (FIXED_TTT, "FixedTtt", DYNAMIC_TTT, "DynamicTtt",
                                          THRESHOLD, "Threshold", NO_AUTOMATIC_HANDOVER, "NoAuto"))
          .AddAttribute ("FixedTttValue", "The value of TTT in case of fixed TTT handover (in ms)",
                         UintegerValue (110), MakeUintegerAccessor (&LteEnbRrc::m_fixedTttValue),
          ...
          .AddAttribute ("MinDynTttValue",
                         ...
                         UintegerValue (25), MakeUintegerAccessor (&LteEnbRrc::m_minDynTttValue),
                         MakeUintegerChecker<uint8_t> ()) // TODO consider using a TimeValue
          .AddAttribute ("MaxDynTttValue",
                         "The maximum value of TTT in case of dynamic TTT handover (in ms)",
                         UintegerValue (150), MakeUintegerAccessor (&LteEnbRrc::m_maxDynTttValue),
                         MakeUintegerChecker<uint8_t> ()) // TODO consider using a TimeValue
          .AddAttribute ("MinDiffValue",
                         "The minimum value of the difference in case of dynamic TTT handover [dB]",
                         DoubleValue (3), MakeDoubleAccessor (&LteEnbRrc::m_minDiffTttValue),
          ...
          .AddAttribute ("MaxDiffValue",
                         ...
                         DoubleValue (20), MakeDoubleAccessor (&LteEnbRrc::m_maxDiffTttValue),
                         MakeDoubleChecker<double> ()) // TODO set the proper value
          .AddAttribute ("CrtPeriod", "The periodicity of a CRT (us)", IntegerValue (1600),
          ...
          .AddAttribute ("ReportAllUeMeas",
                         "If true, the MmWave eNB sends to the LTE coordinator all the received UE "
                         "measures (one per CC). If false, it sends only the maximum measures",
                         BooleanValue (true), MakeBooleanAccessor (&LteEnbRrc::m_reportAllUeMeas),
                         MakeBooleanChecker ())
          // Trace sources
          .AddTraceSource ("NewUeContext", "Fired upon creation of a new UE context.",
          ...
          .AddTraceSource ("ConnectionReconfiguration",
                           ...
                           "ns3::LteEnbRrc::ConnectionHandoverTracedCallback")
          .AddTraceSource ("HandoverStart", "trace fired upon start of a handover procedure",
          ...
          .AddTraceSource ("NotifyMmWaveSinr",
                           "trace fired when measurement report is received from mmWave cells, for "
                           "each cell, for each UE",
...
LteEnbRrc::GetLteFfrRrcSapUser (uint8_t index)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (
      index < m_numberOfComponentCarriers,
      "Invalid component carrier index:" << index << " provided in order to obtain FfrRrcSapUser.");
      ...
      if (false)
      ...
      else
        {
          if (m_ueImsiSinrMap.size () ==
              m_numberOfComponentCarriers) // if we received the ueImsiSinrMap report from all the CCs
            {
              ...
              std::map<uint64_t, double>
                  ueImsiSinrMapToSend; // map which contains the max SINR for each UE among the CCs
              ...
              for (uint8_t cc = 1; cc < m_numberOfComponentCarriers; cc++)
                {
                  ...
                  for (std::map<uint64_t, double>::iterator ue = ueImsiSinrMapToSend.begin ();
                       ue != ueImsiSinrMapToSend.end (); ue++)
                    {
                      NS_ASSERT_MSG (m_ueImsiSinrMap.at (cc).find (ue->first) !=
                                         m_ueImsiSinrMap.at (cc).end (),
                                     "CC " << (uint16_t) cc << " didn't report SINR for UE "
                                           << ue->first);
...
LteEnbRrc::DoRecvUeSinrUpdate (EpcX2SapUser::UeImsiSinrParams params)
{
  ...
  for (std::map<uint64_t, double>::iterator imsiIter = params.ueImsiSinrMap.begin ();
       imsiIter != params.ueImsiSinrMap.end (); ++imsiIter)
    {
  ...
  if (!m_ismmWave && !m_interRatHoMode && m_firstReport)
    {
      ...
      switch (m_handoverMode)
        ...
        case THRESHOLD:
          ...
          break;
        case NO_AUTOMATIC_HANDOVER:
          NS_LOG_INFO ("Handover Mode: No automatic handover");
          break;
        }
...
LteEnbRrc::TttBasedHandover (std::map<uint64_t, CellSinrMap>::iterator imsiIter,
                             double sinrDifference, uint16_t maxSinrCellId, double maxSinrDb)
{
  uint64_t imsi = imsiIter->first;
          ...
          if (handoverEvent->second.targetCellId == maxSinrCellId)
          ...
          else
            {
              ...
              if (maxSinrDb - originalTargetSinrDb >
                  m_sinrThresholdDifference) // this parameter is the same as the one for ThresholdBasedSecondaryCellHandover
                {
                  // delete this event
                  handoverEvent->second.scheduledHandoverEvent.Cancel ();
                  ...
                  if (maxSinrCellId == m_lastMmWaveCell[imsi])
                    {
                      // delete this event
                      NS_LOG_INFO (
                          "-------------- The difference between the two mmWave SINR is smaller "
                          "than "
                          << m_sinrThresholdDifference
                          << " dB, but the new max is the current cell, thus cancel the handover");
                      handoverEvent->second.scheduledHandoverEvent.Cancel ();
                      m_imsiHandoverEventsMap.erase (handoverEvent);
                    }
                  else
  ...
  if (handoverNeeded)
    {
      ...
      // schedule the event
      EventId scheduledHandoverEvent = Simulator::Schedule (
          MilliSeconds (millisecondsToHandover), &LteEnbRrc::PerformHandover, this, imsi);
      ...
      HandoverEventMap::iterator handoverEvent = m_imsiHandoverEventsMap.find (imsi);
      if (handoverEvent !=
          m_imsiHandoverEventsMap
              .end ()) // another event was scheduled, but it was already deleted. Replace the entry
        {
      ...
      if (!onHandoverImsi)
        {
          if (m_lastMmWaveCell[imsi] != handoverInfo.targetCellId)
            ...
            {
              NS_LOG_UNCOND ("## Warn: handover not triggered because target cell id is the same "
                             "of the old cell id!");
            }
        }
      else
...
LteEnbRrc::ThresholdBasedSecondaryCellHandover (std::map<uint64_t, CellSinrMap>::iterator imsiIter,
                                                double sinrDifference, uint16_t maxSinrCellId,
                                                double maxSinrDb)
{
  uint64_t imsi = imsiIter->first;
  bool alreadyAssociatedImsi = false;
          ...
          for (CellSinrMap::iterator cellIter = imsiIter->second.begin ();
               cellIter != imsiIter->second.end (); ++cellIter)
            {
              ...
              if (m_allowHandoverTo.find (cellIter->first)->second)
                ...
                {
                  NS_LOG_DEBUG ("Cell " << cellIter->first << " reports "
                                        << 10 * std::log10 (cellIter->second)
                                        << " but HO not allowed to");
                }
          ...
          if ((maxSinrDb < m_outageThreshold ||
          ...
          else
            {
              ...
              if (m_handoverMode == THRESHOLD)
                {
                  ThresholdBasedSecondaryCellHandover (imsiIter, sinrDifference, maxSinrCellId,
                                                       maxSinrDb);
                }
           ...
           imsiIter != m_imsiCellSinrMap.end (); ++imsiIter)
        {
          ...
          if (maxSinrDb < m_outageThreshold ||
          ...
          else // there is at least a MmWave eNB that can serve this UE
            {
              if (m_handoverMode == THRESHOLD)
                {
                  ThresholdBasedInterRatHandover (imsiIter, sinrDifference, maxSinrCellId,
                                                  maxSinrDb);
                }
              else if (m_handoverMode == FIXED_TTT || m_handoverMode == DYNAMIC_TTT)
                {
...
void
LteEnbRrc::DoRecvRrcSecondaryCellInitialAccessSuccessful (uint16_t rnti, uint16_t mmWaveRnti,
    ...
    EpcEnbS1SapUser::DataRadioBearerSetupRequestParameters request)
{
  ...
  ueManager->SetupDataRadioBearer (request.bearer, request.bearerId, request.gtpTeid,
...
LteEnbRrc::DoRecvHandoverRequest (EpcX2SapUser::HandoverRequestParams req)
{
  ...
  if (anrcrv.valid == false)
    {
      NS_LOG_INFO (
          this << " failed to allocate a preamble for non-contention based RA => cannot accept HO");
      RemoveUe (rnti);
  ...
  for (std::vector<EpcX2Sap::ErabToBeSetupItem>::iterator it = req.bearers.begin ();
       it != req.bearers.end (); ++it)
    {
      ueManager->SetupDataRadioBearer (it->erabLevelQosParameters, it->erabId, it->gtpTeid,
  ...
  if (m_anrSapProvider != 0)
    {
      ...
      NS_LOG_DEBUG (
          this << " cellId=" << ComponentCarrierToCellId (ueManager->GetComponentCarrierId ())
               << " targetCellId=" << targetCellId << " NRT.NoHo=" << noHo << " NRT.NoX2=" << noX2);
...
LteEnbRrc::SetSecondaryCellHandoverAllowedStatus (uint16_t cellId, bool hoAllowed)
{
  ...
  if (entry != m_allowHandoverTo.end ())
    {
      NS_LOG_DEBUG ("HO bar for cell " << cellId << " status from " << entry->second << " to "
                                       << hoAllowed);
  ...
  if (m_imsiCellSinrMap.size () > 0) // there are some entries
    {
      ...
      for (std::map<uint64_t, CellSinrMap>::iterator imsiIter = m_imsiCellSinrMap.begin ();
           imsiIter != m_imsiCellSinrMap.end (); ++imsiIter)
        {
          ...
          if (m_mmWaveCellSetupCompleted.find (imsi) != m_mmWaveCellSetupCompleted.end ())
            {
              ...
              if (m_allowHandoverTo.find (cellId)->second ||
                  (m_allowHandoverTo.find (cellId) == m_allowHandoverTo.end ()))
                {
          ...
          NS_LOG_INFO ("imsi " << imsi << " needs to move from cell " << m_lastMmWaveCell[imsi]
                               << " alreadyAssociatedImsi " << alreadyAssociatedImsi
                               << " onHandoverImsi " << onHandoverImsi);
...
LteEnbRrc::GetM_ueImsiSinrMap ()
{
  return m_ueImsiSinrMap;
}
