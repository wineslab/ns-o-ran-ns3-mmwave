...
UeManager::SwitchToState(State newState)
{
    ...
    m_state = newState;
    NS_LOG_INFO(this << " ENB Switch to state IMSI " << m_imsi << " RNTI " << m_rnti
                     << " UeManager " << ToString(oldState) << " --> " << ToString(newState));
...
LteEnbRrc::DoRecvUeSinrUpdate(EpcX2SapUser::UeImsiSinrParams params)
{
    ...
    // add the cell to the list of cells that need to be tracked for on/off state and handover
    // barring
