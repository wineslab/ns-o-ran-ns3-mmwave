...
class EnergyHeuristic : public Object{
  ...
  /**
   * @brief Method that decides which mmWave BSs to turn idle, sleep and off based on the smallest time that connected ues take to reach them
   * 
   * @param bsToTurnOn Array that contains a list mmWave BS indexes that are turned on. Each BS is identified with an index value used to save them in mmWaveEnbDevs
   ...
   * @return std::vector<std::pair<Ptr<MmWaveEnbNetDevice>, double>> Vector of pairs: the mmWave Bs and the time that takes the closest UE to reach it
   */
  ...
  /**
   * @brief Method that decides which mmWave BSs to turn idle, sleep and off based on the distance of the closest ue 
   * 
   * @param bsToTurnOn Array that contains a list mmWave BS indexes that are turned on. Each BS is identified with an index used to save them in mmWaveEnbDevs
   * @param bsOn Number of BSs that are turned on
   * @param bsIdle Number of BSs that are turned idle
   * @param bsSleep Number of BSs that are turned sleep
   * @param bsOff Number of BSs that are turned off
   * @param nMmWaveEnbNodes Number of mmWave BSs in the scenario
   * @param mmWaveEnbDevs Object that contains all the mmWave BSs
   * @return std::vector<std::pair<Ptr<MmWaveEnbNetDevice>, double>> Vector of pairs: the mmWave Bs and the distance of the closest UE
   */
